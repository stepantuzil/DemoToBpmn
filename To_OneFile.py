    ...
    for number_of_transaction in list(range(len(TransactionKinds_arr))):
        if number_of_transaction != 0:
            between_message_flow = '     <bpmn:messageFlow id="MessageFlow_F_' + str(number_of_transaction) \
                                   + '" sourceRef="IntermediateThrowEvent_0_' + TransactionKinds_arr[number_of_transaction-1].identification \
                                   + '" targetRef="IntermediateCatchEvent_3_' + TransactionKinds_arr[number_of_transaction].identification + '" />\n'
...
######################################
    ...
    for line in f_diagrams:
        if line.find("<bpmndi:BPMNDiagram") == -1 & line.find("</bpmndi:BPMNDiagram>") == -1 & line.find("<bpmndi:BPMNPlane") == -1 & line.find("</bpmndi:BPMNPlane") == -1:
            f_final.write(line)
